...
class AgentRouterTest extends TestCase
{
    ...
    public function test_a_prospect_scoring_question_routes_to_market_intelligence(): void
    {
        $this->assertSame(AgentIdentifier::MarketIntelligence, (new AgentRouter)->route('Score these prospects and rank them.'));
        $this->assertSame(AgentIdentifier::MarketIntelligence, (new AgentRouter)->route('Which prospects deserve the most business-development attention?'));
        $this->assertSame(AgentIdentifier::MarketIntelligence, (new AgentRouter)->route('Rank the cosmetics prospects by priority band.'));
        $this->assertSame(AgentIdentifier::MarketIntelligence, (new AgentRouter)->route('Give me a prospect score for the businesses in Cebu.'));
    }

    public function test_a_crm_duplicate_check_question_routes_to_market_intelligence(): void
    {
        $this->assertSame(AgentIdentifier::MarketIntelligence, (new AgentRouter)->route('Run a duplicate check on these prospects.'));
        $this->assertSame(AgentIdentifier::MarketIntelligence, (new AgentRouter)->route('Are these prospects already in the CRM?'));
        $this->assertSame(AgentIdentifier::MarketIntelligence, (new AgentRouter)->route('Do we already have ABC Beauty as a lead?'));
        $this->assertSame(AgentIdentifier::MarketIntelligence, (new AgentRouter)->route('Check these businesses for existing CRM records before we add them.'));
    }

    public function test_a_prospect_to_crm_request_routes_to_market_intelligence(): void
    {
        $this->assertSame(AgentIdentifier::MarketIntelligence, (new AgentRouter)->route('Add this prospect to the CRM.'));
        $this->assertSame(AgentIdentifier::MarketIntelligence, (new AgentRouter)->route('Turn this prospect into a lead.'));
        $this->assertSame(AgentIdentifier::MarketIntelligence, (new AgentRouter)->route('Prepare the prospect for CRM creation.'));
        $this->assertSame(AgentIdentifier::MarketIntelligence, (new AgentRouter)->route('Create a lead from this prospect research.'));
    }
